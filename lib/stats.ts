...
export interface Progress { key: string; total: number; notCalled: number; notPicked: number; connected: number; pending: number; attempts: number }
...
interface Row { key: string; status: string; pending: boolean; count: number; attempts: number }
...
async function rows(orgId: string, f: Filters, dim: Dimension): Promise<Row[]> {
  ...
  if (isDateDim(dim)) {
    const g = await prisma.lead.groupBy({ by: ["queryDate", "status", "remarkPending"], where: { AND: [where, { queryDate: { not: null } }] }, _count: { _all: true }, _sum: { callAttempts: true } });
    return g.map((r) => ({ key: dateKey(r.queryDate!, dim), status: r.status, pending: r.remarkPending, count: r._count._all, attempts: r._sum.callAttempts ?? 0 }));
  }
  const g = (await prisma.lead.groupBy({ by: [dim, "status", "remarkPending"] as never, where, _count: { _all: true }, _sum: { callAttempts: true } } as never)) as unknown as Array<
    Record<string, string | null> & { status: string; remarkPending: boolean; _count: { _all: number }; _sum: { callAttempts: number | null } }
  >;
  return g.filter((r) => r[dim]).map((r) => ({ key: r[dim] as string, status: r.status, pending: r.remarkPending, count: r._count._all, attempts: r._sum.callAttempts ?? 0 }));
}
...
export async function callProgress(orgId: string, f: Filters, by: Dimension): Promise<Progress[]> {
  ...
  for (const r of await rows(orgId, f, by)) {
    const p = map.get(r.key) ?? { key: r.key, total: 0, notCalled: 0, notPicked: 0, connected: 0, pending: 0, attempts: 0 };
    p.total += r.count;
    p.attempts += r.attempts;
    if (r.pending) p.pending += r.count;
    else if (r.status === "NEW") p.notCalled += r.count;
...
export async function getStats(orgId: string, f: Filters, by: Dimension = "source") {
  ...
  return {
    kpis: { total, contacted, interested, won, dueToday, hotUntouched: hot, pendingRemarks },
